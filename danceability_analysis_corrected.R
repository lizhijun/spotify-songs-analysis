...
# Using regression and correlation analyses - CORRECTED VERSION
...
# Display raw correlations (not absolute values)
cat("Raw Correlations with Danceability:\n")
sorted_raw_correlations <- sort(danceability_correlations, decreasing = TRUE)
for(i in 1:length(sorted_raw_correlations)) {
  feature <- names(sorted_raw_correlations)[i]
  correlation <- sorted_raw_correlations[i]
  cat(sprintf("%-15s: %6.3f\n", feature, correlation))
}
cat("\n")

# Sort correlations by absolute value for visualization purposes
...
# Scatter plots for top correlations (using raw correlations, not absolute)
top_features_raw <- names(head(sorted_raw_correlations, 4))

scatter_plots <- list()
for(i in 1:length(top_features_raw)) {
  feature <- top_features_raw[i]
...
ggsave("danceability_scatter_plots_corrected.png", combined_scatter, 
...
ggsave("feature_importance_corrected.png", importance_plot, 
...
# 6. Summary insights
cat("=== KEY INSIGHTS ===\n")
cat("Features that INCREASE danceability (based on regression):\n")
...
cat("\nFeatures that DECREASE danceability (based on regression):\n")
...
cat("\nTop positive correlations with danceability:\n")
top_positive <- sorted_raw_correlations[sorted_raw_correlations > 0]
for(i in 1:length(top_positive)) {
  feature <- names(top_positive)[i]
  correlation <- top_positive[i]
  cat(sprintf("- %s: %.3f\n", feature, correlation))
}

cat("\nTop negative correlations with danceability:\n")
top_negative <- sorted_raw_correlations[sorted_raw_correlations < 0]
top_negative <- sort(top_negative)
for(i in 1:length(top_negative)) {
  feature <- names(top_negative)[i]
  correlation <- top_negative[i]
  cat(sprintf("- %s: %.3f\n", feature, correlation))
}
...
cat("2. danceability_scatter_plots_corrected.png - Scatter plots of top positively correlated features\n")
cat("3. feature_importance_corrected.png - Regression coefficients visualization\n")
