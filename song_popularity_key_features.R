# 歌曲受欢迎程度预测 - 关键特征分析
# Key Features Analysis for Song Popularity Prediction
...
audio_coefficients <- tidy(model) %>%
  ...
  mutate(
    ...
    effect_type = ifelse(estimate > 0, "正面影响", "负面影响")
  )
...
all_coefficients <- bind_rows(
  audio_coefficients %>% 
    mutate(feature_type = "音频特征", clean_name = term),
  genre_coefficients %>% 
    mutate(feature_type = "音乐流派", clean_name = genre)
) %>%
  filter(p.value < 0.05) %>%  # 只考虑显著特征
...
combined_plot <- ggplot(combined_features, aes(x = reorder(display_name, estimate), 
  ...
  coord_flip() +
  scale_fill_manual(values = c("音频特征" = "#2E86AB", "音乐流派" = "#A23B72")) +
  labs(
...
ggsave("key_features_for_popularity.png", combined_plot, 
       width = 12, height = 8, dpi = 300, bg = "white")

# 5. 散点图展示最重要特征与受欢迎程度的关系
# 获取最重要的音频特征
...
for(i in 1:length(top_audio_features)) {
  ...
  feature_cn <- ifelse(feature %in% names(feature_names_cn), 
                      feature_names_cn[feature], 
                      feature)
...
ggsave("key_features_scatter_plots.png", combined_scatter, 
...
genre_impact <- genre_coefficients %>%
  ...
  mutate(
    genre_cn = case_when(
      grepl("edm", genre) ~ "电子舞曲",
      grepl("latin", genre) ~ "拉丁音乐", 
      grepl("pop", genre) ~ "流行音乐",
      grepl("r&b", genre) ~ "节奏布鲁斯",
      grepl("rap", genre) ~ "说唱音乐",
      grepl("rock", genre) ~ "摇滚音乐",
      TRUE ~ genre
    ),
...
# 流派影响图
genre_plot <- ggplot(genre_impact, aes(x = reorder(genre_cn, estimate), 
                                      y = estimate, 
                                      fill = color_group)) +
  geom_col(alpha = 0.8, width = 0.7) +
  geom_text(aes(label = paste0(round(estimate, 2), significance)), 
...
ggsave("genre_impact_on_popularity.png", genre_plot, 
       width = 12, height = 8, dpi = 300, bg = "white")
...
# 创建相关性热力图
png("features_correlation_heatmap.png", width = 10, height = 8, units = "in", res = 300)
...
for(i in 1:min(3, nrow(top_positive_features))) {
  ...
  cat(sprintf("   影响系数: +%.2f (p < %.5f)\n", coefficient, p_value))
  cat(sprintf("   实际意义: 该特征每增加一个单位，受欢迎程度平均增加%.2f分\n\n", coefficient))
}

cat("生成的可视化文件:\n")
cat("1. key_features_for_popularity.png - 最重要特征综合排序\n")
cat("2. key_features_scatter_plots.png - 重要特征散点图\n")
cat("3. genre_impact_on_popularity.png - 流派影响分析\n")
cat("4. features_correlation_heatmap.png - 特征相关性热力图\n")